...
| R21 | `REBUILD_PLAN.md:126` lists `Dataset/Test/{CHAMELEON,NC4K}/Imgs` as **"Test, secondary"** in its data-provenance table | `EXP D2R`, T13 | Checked against the repo's own README for the first time | `README.md` mentions CHAMELEON **once**, inside the `Source (Synthetic)` CNC bundle, and lists only COD10K-test under `Test (Real)`. `Dataset/Source/CNC/` does not exist on disk; the undocumented `Dataset/Test/CHAMELEON/` does. `REPRODUCE_TABLE1_v2.md` has **0** CHAMELEON mentions and `Result/**` holds **0** CHAMELEON predictions. **We promoted CHAMELEON to an endpoint the repository never sanctioned.** The framing is therefore not "a bug in their evaluation" — CHAMELEON is *source* material in this design |
...
| CHAMELEON contaminated by training data | 41/76 (53.9 %) | **superseded — now 50/76 (65.8 %)**; see note below | `EXP D2R`, then `rebuild/D2_FINAL_AUDIT/` |

> **This row moved after this file froze.** `41/76` was the JPEG-re-encode count. The geometric extension (crops, rescales) raised it, and the two counts were briefly reported as `51/76` and `50/76` because the same integer meant different things in different artifacts. `rebuild/D2_FINAL_AUDIT/CONTAMINATION_LEDGER.md` adjudicates every integer this project has produced about CHAMELEON and settles it at **50 of 76 (65.8 %)**, which is what the paper reports. Read that ledger, not this row.
