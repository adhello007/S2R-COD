...
from Src.model.SegMaR.ResNet import B2_ResNet
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
from torch.nn import Parameter, Softmax
import torch.nn.functional as F
from Src.model.SegMaR.HolisticAttention import HA
...
class Saliency_feat_encoder(nn.Module):
    ...
    def initialize_weights(self):
        # Upstream SegMaR used models.resnet50(pretrained=True), which in 2022 resolved to
        # IMAGENET1K_V1. We deliberately reuse the local IMAGENET1K_V2 file that SINet uses
        # here, so SegMaR's numbers stay comparable to the already-reproduced SINet rows.
        # B2_ResNet's layer names are identical to SINet's ResNet_2Branch, so the same
        # _1/_2 key remap below applies unchanged. See Explanations/SEGMAR.md.
        pretrained_dict = torch.load('Src/model/SINet/resnet50-11ad3fa6.pth',
                                     map_location='cpu')
        all_params = {}
