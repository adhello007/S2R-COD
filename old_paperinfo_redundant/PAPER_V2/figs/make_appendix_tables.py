...
OUT = os.path.join(REPO, 'rebuild', 'PAPER_V2', 'appendix_tables.tex')
...
def runs(path, tag):
    ...
    for r in csv.DictReader(open(path)):
        if r['endpoint'] == 'COD10K':
            out.append((tag, r['arch'], r['arm'], r['seed'], float(r['Sm']), float(r['MAE'])))
    return out
...
\midrule""")
for tag, arch, arm, seed, sm, mae in sorted(uniq, key=lambda t: (t[1], t[2], t[3])):
    w('%s & %s & %s & %.6f & %.6f \\\\' % (arch, arm, seed, sm, mae))
w(r"""\bottomrule
\end{tabular}
\end{center}
\end{table}
