...
Selection rule, declared in the caption and applied here: the THREE LOWEST and
the THREE MOST MARGINAL mean|diff| among pairs whose partner lies in
cod10k_train. Showing only the closest pairs would show only the cases a
skeptic doubts least; the marginal half is the honest other end, and the most
marginal pair sits at the nearest-neighbour gap boundary. Nothing is picked by
eye; the ordering is the committed metric.
...
OUT = os.path.join(REPO, 'rebuild', 'PAPER_V4', 'figs', 'fig2_pairs.pdf')
...
_ok = sorted([p for p in pairs if p['partner_pool'] == 'cod10k_train'],
             key=lambda r: r['mean_abs'])
pairs = _ok[:3] + _ok[-3:]          # three closest, three most marginal

fig, axes = plt.subplots(2, N, figsize=(11.0, 1.62))
for j, p in enumerate(pairs):
    ...
    if j >= 3:
        for _s in axes[0][j].spines.values():
            _s.set_linestyle((0, (3, 2)))
        for _s in axes[1][j].spines.values():
            _s.set_linestyle((0, (3, 2)))
