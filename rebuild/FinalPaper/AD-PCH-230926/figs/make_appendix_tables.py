...
OUT = paper_out('appendix_tables.tex')
...
# Split by endpoint: one 78-row float overflows the page and truncates the NC4K rows.
PERRUN_HEAD = r"""\begin{center}\small
\begin{tabular}{llllcc}
\toprule
arch & arm & seed & endpoint & $S_\alpha$ & MAE \\
\midrule"""
PERRUN_CAPTION = {
    'COD10K': r"""\caption{Per-run metrics at full recorded precision, COD10K-test endpoint, for every run of the two
frozen campaigns and the mean-teacher positive control --- $%d$ of the $103$ training runs in this
...
\label{tab:perrun}""",
    'NC4K': r"""\caption{Per-run metrics at full recorded precision, NC4K endpoint, for the same $%d$ runs as
Table~\ref{tab:perrun}. \emph{Source:} as Table~\ref{tab:perrun}.}
\label{tab:perrun-nc4k}""",
}
for endpoint in ('COD10K', 'NC4K'):
    sel = [t for t in uniq if t[4] == endpoint]
    w(r'\begin{table}[p]')
    w(PERRUN_CAPTION[endpoint] % len(sel))
    w(PERRUN_HEAD)
    for tag, arch, arm, seed, ep, sm, mae in sorted(sel, key=lambda t: (t[4], t[1], t[2], t[3])):
