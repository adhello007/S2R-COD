...
right: trained S_alpha per C-family arm with the frozen 2*sigma_hat bar, at
       EIGHT seeds (EXP SE, out/se/abc_metrics.csv + abc_sigma.json).  The
       three arms shown reproduce one allocation's shape to five decimals and
       differ only in where the budget points.
"""
...
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'fig1_thesis.pdf')
...
met = list(csv.DictReader(open(os.path.join(REPO, 'rebuild/ABC/out/se/abc_metrics.csv'))))
sig = json.load(open(os.path.join(REPO, 'rebuild/ABC/out/se/abc_sigma.json')))
...
for ai, (akey, aname) in enumerate(ARCH):
    ...
    for arm in CARMS:
        ...
        pos += 1
    axR.text(pos - 2, 0.7278, aname, ha='center', fontsize=9)
    pos += 1
axR.set_xlim(-0.7, pos - 1.3)
axR.set_xticks(tick); axR.set_xticklabels(lab, fontsize=7.2)
axR.set_ylim(0.6820, 0.7300)
axR.set_ylabel(r'COD10K-test $S_\alpha$', fontsize=9)
axR.set_title('(b) At eight seeds, real, destroyed and reversed\ntargeting are still indistinguishable',
              fontsize=9.5, pad=6)
axR.tick_params(axis='y', labelsize=8)
axR.legend(fontsize=6.8, loc='lower left', framealpha=0.95, ncol=1, borderpad=0.35)
for s_ in ('top', 'right'):
